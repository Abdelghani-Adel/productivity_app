...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    try {
      ...
      const quotesCollections = db.collection("qutoes");
      /** Inserting the new document in the collection */
      const result = await quotesCollections.insertOne(JSON.parse(data));

      /** Selecting the collection again after inserting to get the updated one */
      const updatedCollection = db.collection("qutoes");
